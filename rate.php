...
connect();
//$q=mysql_num_rows(mysql_query("select id from usa where id=$id_sent"));
if ($vote_sent > $units) die("Sorry, vote appears to be invalid."); // kill the script because normal users will never see this.


//connecting to the database to get some information
$query = mysql_query("SELECT total_votes, total_value, used_ips, user_voted FROM ".$rating_dbname.".".$rating_tableName." WHERE id='$id_sent' ")or die(" Error2: ".mysql_error());
$numbers = mysql_fetch_assoc($query);
$checkIP = unserialize($numbers['used_ips']);
$user_voted = unserialize($numbers['used_ips']);
...
((is_array($user_voted)) ? array_push($user_voted,$user) : $user_voted=array($user));
$user_voted = serialize($user_voted);
$user_voted = $user;
...
if(!$voted) {     //if the user hasn't yet voted, then vote normally...
	...
	if (($vote_sent >= 1 && $vote_sent <= $units)) { // keep votes within range, make sure IP matches 
		$rez = $sum/$added;
		$update = "UPDATE ".$rating_tableName." SET total_votes='".$added."', total_value='".$sum."', used_ips='".$insertip."', user_voted='".$user_voted."' , rez='".$rez."'WHERE id='$id_sent'";
		$result = mysql_query($update);	
		if($result)	setcookie("rating_".$id_sent,1, time()+ 2592000);
	} 
} //end for the "if(!$voted)"
// these are new queries to get the new values!
$newtotals = mysql_query("SELECT total_votes, total_value, used_ips, user_voted FROM ".$rating_dbname.".".$rating_tableName." WHERE id='$id_sent' ")or die(" Error3: ".mysql_error());
